...
namespace WFCalendarApp {
    ...
    static class EventUtils {
        ...
        public static readonly Color OFFICE_COLOR = Color.FromArgb(19, 143, 17);
        public static readonly Color OOO_COLOR = Color.FromArgb(65, 145, 240);
        public static readonly Color NO_EVENTS_COLOR = Color.FromArgb(209, 38, 33);
        public static readonly Color SEARCHING_COLOR = Color.FromArgb(250, 191, 143);
        public static readonly Color NON_WORK_COLOR = Color.FromArgb(230, 230, 230);
        ...
        //Does the same thing as everything else, just a different way
        //To implement new features, the easiest thing to do was copy and paste old code
        //Rather than rewrite what some other programmer wrote
        public static Dictionary<Employee, List<TimePeriod>> LookingDict(
        Dictionary<Employee, IList<GCEvent>> data, DateTime start,
        DateTime end)
        {
            var result = new Dictionary<Employee, List<TimePeriod>>(data.Count);
            foreach (var pair in data)
            {
                var employee = pair.Key;
                var events = pair.Value;
                List<TimePeriod> periods = GetTimePeriods(events, start, end);
                List<TimePeriod> tempList = new List<TimePeriod>();
                foreach (var period in periods)
                {
                    if ((period.Type == EventType.NO_EVENTS || period.Type == EventType.SEARCHING) && period.Type != EventType.OOO)
                    {
                        tempList.Add(period);
                    }
                }
                if (tempList.Count > 0)
                {
                    result.Add(employee, periods);
                }
            }
            return result;
        }

        //And again here.  Copy and paste, change some things.  All these are referenced in an if conditional in MainForm
        public static Dictionary<Employee, List<TimePeriod>> InOfficeDict(Dictionary<Employee, IList<GCEvent>> data, DateTime start, DateTime end)
        {
            var result = new Dictionary<Employee, List<TimePeriod>>(data.Count);
            foreach (var pair in data)
            {
